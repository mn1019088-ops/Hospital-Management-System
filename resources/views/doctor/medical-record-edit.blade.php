...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    ...
    <div>
        <a href="{{ route('doctor.medical-records.show', $medicalRecord->id) }}" class="btn btn-info me-2">
...
<form action="{{ route('doctor.medical-records.update', $medicalRecord->id) }}" method="POST">
    ...
    <div class="card border-secondary shadow-sm mb-4">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-4">
                    <strong>Created:</strong> {{ $medicalRecord->created_at->format('M d, Y \\a\\t h:i A') }}
                </div>
                <div class="col-md-4">
                    <strong>Last Updated:</strong> {{ $medicalRecord->updated_at->format('M d, Y \\a\\t h:i A') }}
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="visit_date" id="visit_date" class="form-control @error('visit_date') is-invalid @enderror"
                        value="{{ old('visit_date', $medicalRecord->visit_date->format('Y-m-d')) }}" max="{{ now()->format('Y-m-d') }}" required>
                    @error('visit_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
    ...
    <div class="card border-info shadow-sm mb-4">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-3 mb-3">
                    ...
                    <input type="number" name="weight" id="weight" class="form-control @error('weight') is-invalid @enderror" 
                        step="0.1" min="1" max="300" value="{{ old('weight', $medicalRecord->weight) }}" required>
                    @error('weight')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-3 mb-3">
                    ...
                    <input type="number" name="height" id="height" class="form-control @error('height') is-invalid @enderror" 
                        step="0.1" min="30" max="250" value="{{ old('height', $medicalRecord->height) }}" required>
                    @error('height')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-3 mb-3">
                    ...
                    <input type="text" name="blood_pressure" class="form-control @error('blood_pressure') is-invalid @enderror" 
                        placeholder="e.g., 120/80" value="{{ old('blood_pressure', $medicalRecord->blood_pressure) }}" required>
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="number" name="temperature" class="form-control @error('temperature') is-invalid @enderror" 
                        step="0.1" min="35" max="42" value="{{ old('temperature', $medicalRecord->temperature) }}" required>
                    @error('temperature')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row">
                <div class="col-md-3 mb-3">
                    ...
                    <input type="number" name="pulse_rate" class="form-control @error('pulse_rate') is-invalid @enderror" 
                        min="30" max="200" value="{{ old('pulse_rate', $medicalRecord->pulse_rate) }}" required>
                    @error('pulse_rate')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-3 mb-3">
                    ...
                    <input type="number" name="respiratory_rate" class="form-control @error('respiratory_rate') is-invalid @enderror" 
                        min="8" max="40" value="{{ old('respiratory_rate', $medicalRecord->respiratory_rate) }}" required>
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="number" name="oxygen_saturation" class="form-control @error('oxygen_saturation') is-invalid @enderror" 
                        min="70" max="100" step="0.1" value="{{ old('oxygen_saturation', $medicalRecord->oxygen_saturation) }}">
            ...
            <!-- BMI Calculation -->
            <div class="row mb-3" id="bmiCalculation">
                <div class="col-12">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row text-center">
                                <div class="col-md-4">
                                    ...
                                    <span id="displayWeight" class="h5">{{ number_format($medicalRecord->weight, 1) }}</span> kg
                                </div>
                                <div class="col-md-4">
                                    <strong>Height:</strong><br>
                                    <span id="displayHeight" class="h5">{{ number_format($medicalRecord->height, 1) }}</span> cm
                                </div>
                                <div class="col-md-4">
                                    <strong>BMI:</strong><br>
                                    <span id="displayBMI" class="h5">{{ number_format($medicalRecord->bmi, 1) }}</span><br>
                                    <span id="bmiCategory" class="badge bg-{{ $medicalRecord->bmi_category == 'Normal' ? 'success' : ($medicalRecord->bmi_category == 'Underweight' ? 'warning' : ($medicalRecord->bmi_category == 'Overweight' ? 'warning' : 'danger')) }}">
                                        {{ $medicalRecord->bmi_category }}
                                    </span>
                                </div>
                            </div>
                        </div>
        ...
        <div class="card-body">
            <div class="mb-3">
                ...
                <textarea name="symptoms" class="form-control @error('symptoms') is-invalid @enderror" 
                    rows="3" placeholder="Describe the patient's symptoms..." required>{{ old('symptoms', $medicalRecord->symptoms) }}</textarea>
                @error('symptoms')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <textarea name="diagnosis" class="form-control @error('diagnosis') is-invalid @enderror" 
                    rows="3" placeholder="Enter the diagnosis..." required>{{ old('diagnosis', $medicalRecord->diagnosis) }}</textarea>
                @error('diagnosis')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <textarea name="treatment" class="form-control @error('treatment') is-invalid @enderror" 
                    rows="3" placeholder="Describe the treatment provided..." required>{{ old('treatment', $medicalRecord->treatment) }}</textarea>
                @error('treatment')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <textarea name="prescription" class="form-control @error('prescription') is-invalid @enderror" 
                    rows="3" placeholder="List prescribed medications and dosage...">{{ old('prescription', $medicalRecord->prescription) }}</textarea>
                @error('prescription')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" 
                    rows="2" placeholder="Any additional observations or recommendations...">{{ old('notes', $medicalRecord->notes) }}</textarea>
                @error('notes')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize patient info on page load
        updatePatientInfo();
